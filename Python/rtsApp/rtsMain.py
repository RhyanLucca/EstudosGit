...
class Aplication():
    ...
    def login_frame(self):
        ...
        #     self.loginFrame.rowconfigure(row, weight=1)
        # for column in range(columns):
        #     self.loginFrame.columnconfigure(column,weight=1)

        # ctk.CTkLabel(master=self.loginFrame, text="Insira o nome de usuario").grid(row=2, column=1)

        ctk.CTkLabel(master=loginFrame, text="Usuário", font=(f"{self.font}", self.scrWidth/40)).place(relx=0.1, rely=0.1)

        userEntry= ctk.CTkEntry(master=loginFrame, placeholder_text="Insira o usuário", width=scrWidth/1.2 ,font=(f"{self.font}", self.scrWidth/40)).place(relx=0.1, rely=0.25)

        ctk.CTkLabel(master=loginFrame, text="Senha", font=(f"{self.font}", self.scrWidth/40)).place(relx=0.1, rely=0.4)

        userEntry= ctk.CTkEntry(master=loginFrame, placeholder_text="Insira a senha", width=scrWidth/1.2 ,font=(f"{self.font}", self.scrWidth/40)).place(relx=0.1, rely=0.55)
