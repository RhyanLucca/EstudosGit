...
class Aplication():
    ...
    def login_frame(self):
        ...
        self.font = "Arial Bold"
        loginFrame= ctk.CTkFrame(master=self.geralFrame, width=scrWidth, height=scrHeight)
        loginFrame.place(relx=0.5, rely=0.5, anchor=CENTER)

        currentFrame=self.geralFrame

        self.userPermissao = []


        def openMsg(message, color):
                global frameMsg
                frameMsg = ctk.CTkFrame(master=currentFrame, width=self.app.winfo_screenwidth(), height=self.app.winfo_screenheight()/20, corner_radius=False, fg_color=f"{color}")
                msg = ctk.CTkLabel(master=frameMsg, text=f"{message}", font=(f"{self.font}", self.scrWidth/50))
                print(f'master=frameMsg, text=f"{message}", font=(f"{self.font}", self.scrWidth/50))')
                frameMsg.pack(side="bottom", fill="x")
                msg.pack()
                frameMsg.after(2000, deleteMsg)
        ...
        def loginUser():
            ...
            login = rts.buscarUsuario(entryUser, entryPswd)

            if login[0] == True:

                msg= f"Bem vindo {entryUser}."
                color = "green"
        ...
        btnLogin = ctk.CTkButton(master=loginFrame, text="Entrar", command=loginUser, width=scrWidth/2, font=(f"{self.font}", self.scrWidth/40)).place(relx=0.1, rely=0.8)
